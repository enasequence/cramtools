...
public class ReferenceTracks {
	...
	private ReferenceTracks(final int sequenceId, final String sequenceName, final byte[] reference,
	...
	void moveForwardTo(int newPos) {
		System.out.println("moving tracks to " + newPos);
	...
	public void ensureRange(final int start, final int length) {
		if (start < position)
			throw new RuntimeException("Cannot move the window backwards: " + start);
