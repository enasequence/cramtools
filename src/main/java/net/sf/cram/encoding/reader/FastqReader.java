...
public class FastqReader implements Iterator<FastqRead> {
	...
	private byte[] readName() throws IOException {
		...
		if (stripSegmentIndexFromReadName) {
			...
			case '/':
				...
				if (indexInSegment > 0) {
					is.reset();
					name = IOUtils.readFully(is, nameLen - 2);
		...
		if (name == null) {
			is.reset();
			name = IOUtils.readFully(is, nameLen);
			// skip new line byte:
			is.skip(1);
		}
	...
	private byte[] readBases() throws IOException {
		...
		byte[] bases = IOUtils.readFully(is, readLen);
		// skip new line byte:
		is.skip(1);
	...
	private FastqRead readNext() throws IOException {
		...
		is.skip(1 + 1);
		byte[] scores = IOUtils.readFully(is, bases.length);
