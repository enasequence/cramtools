...
public class Bam2Cram {
	...
	public static void updateTracks(List<SAMRecord> samRecords, ReferenceTracks tracks) {
		for (SAMRecord samRecord : samRecords) {
			if (samRecord.getAlignmentStart() != SAMRecord.NO_ALIGNMENT_START && samRecord.getReadBases().length > 0) {
	...
	public static List<CramRecord> convert(List<SAMRecord> samRecords, SAMFileHeader samFileHeader, byte[] ref,
			...
			if (samFileHeader.getSortOrder() == SAMFileHeader.SortOrder.coordinate) {
				...
				for (CramRecord r : cramRecords) {
					if (!r.isMultiFragment()) {
						...
						Map<String, CramRecord> mateMap = r.isSecondaryAlignment() ? secondaryMateMap : primaryMateMap;
	...
	public static void main(String[] args) throws IOException, IllegalArgumentException, IllegalAccessException,
		...
		ContainerFactory cf = new ContainerFactory(samFileHeader, params.maxContainerSize, params.preserveReadNames);
		...
		do {
			...
			if (samRecord == null || samRecord.getReferenceIndex() != prevSeqId
				...
				if (!samRecords.isEmpty()) {
					...
					List<CramRecord> records = convert(samRecords, samFileHeader, ref, tracks, preservation,
					...
					for (Slice s : container.slices) {
						...
						for (Integer i : s.external.keySet()) {
							...
							long value = externalBytes.get(i) + s.external.get(i).getCompressedContent().length;
		...
		for (int i : externalBytes.keySet())
			if (externalBytes.get(i) > 0)
				sb.append(String.format(", ex%d %.2f b/b, ", i, 8f * externalBytes.get(i) / bases));
