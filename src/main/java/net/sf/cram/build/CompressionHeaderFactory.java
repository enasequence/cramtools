...
public class CompressionHeaderFactory {
	...
	public CompressionHeader build(List<CramRecord> records, SubstitutionMatrix substitutionMatrix, boolean sorted) {
		...
		h.externalCompressors.put(baseID, ExternalCompressor.createRANS(ORDER.ONE));

		int qualityScoreID = exCounter++;
		h.externalIds.add(qualityScoreID);
		h.externalCompressors.put(qualityScoreID, ExternalCompressor.createRANS(ORDER.ONE));
		...
		h.externalCompressors.put(mateInfoID, ExternalCompressor.createRANS(ORDER.ONE));
		...
		if (sorted) { // alignment offset:
			...
			h.eMap.put(EncodingKey.AP_AlignmentPositionOffset, ExternalIntegerEncoding.toParam(aStartID));
			h.externalIds.add(aStartID);
			h.externalCompressors.put(aStartID, ExternalCompressor.createRANS(ORDER.ONE));
		...
		{ // tag values
			...
			h.externalCompressors.put(unsortedTagValueExternalID, ExternalCompressor.createRANS(ORDER.ONE));
			...
			for (int id : tagIdSet) {
				...
				h.externalCompressors.put(externalID, ExternalCompressor.createRANS(ORDER.ONE));
				h.tMap.put(
						id,
						ByteArrayLenEncoding.toParam(ExternalIntegerEncoding.toParam(externalID),
